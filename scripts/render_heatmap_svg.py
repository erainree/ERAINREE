...
Renders data/contributions.json (produced by fetch_contributions.py) as a
GitHub-style contribution heatmap SVG: a 53-week x 7-day grid of rounded
boxes, revealed once with a diagonal line-after-line slide-down, a
Less -> More legend, and a stats footer. Styled to match the terminal
look of the ASCII portrait and info card (same dark background, frame,
and accent colors).

Run by .github/workflows/update-profile-art.yml after fetch_contributions.py.

Usage:
    python3 render_heatmap_svg.py
Output:
    ../contrib-heatmap.svg
"""
...
# same palette as the ASCII portrait / info card, for visual consistency
BG = "#0d1117"
BG2 = "#111722"
FRAME = "#30363d"
MUTED = "#7d8590"
...
# reveal timing (one-shot, diagonal)
COL_T = 0.018
ROW_T = 0.045
CELL_DUR = 0.42
...
""".strip()
    ...
    parts.append(
        ...
        f'text-anchor="middle">elaine@github: ~$ ./contributions.sh</text>'
    )
    ...
    for ci, column in enumerate(grid):
        ...
        for ri, cell in enumerate(column):
            ...
            parts.append(
                f'<rect{cls} x="{gx}" y="{gy}" width="{CELL}" height="{CELL}" rx="2.5" '
                f'fill="{PALETTE[lvl]}"{style}>'
